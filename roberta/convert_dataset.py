...
# For now, the label is only a single skill. In the future, make this a list?
def convert_dataset(dataset, use_multi_label=False):
    ...
    for record in dataset:
        ...
        if len(skill_list) == 0:
            continue
        # multi label -> list of ints, single label -> single int
        if use_multi_label:
            data["label"] = [LABEL2ID[skill] for skill in skill_list]
        else:
            data["label"] = LABEL2ID[skill_list[0]]
